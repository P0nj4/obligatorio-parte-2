...
public class Exercise implements IExercise {
	...
	public static void main(String[] args) {
		...
		prueba
				.testAll("------------- INGRESAR NUMEROS DE ESTUDIANTES -------------");
	}
	 ...
	 * */
	@Override
	public void explorador(int iOrigen, int jOrigen, int iDestino,
			int jDestino, int nivelDeEnergiaActual, int numPasos, int maxPasos,
		...
		explorador(iOrigen, jOrigen, iDestino, jDestino, numPasos, maxPasos,
				nivelDeEnergiaActual, datos, caminoActual, mejoresCaminos,
				mapa, n, m);
	...
	private void explorador(int iOrigen, int jOrigen, int iDestino,
			int jDestino, int numPasos, int maxPasos, int nivelDeEnergiaActual,
			Util datos, ILista caminoActual, ILista mejoresCaminos,
			int[][] mapa, int n, int m) {
		if (PerteneceAlMapa(n, m, iOrigen, jOrigen)) {
			if (!Pase(caminoActual, iOrigen, jOrigen)) {
				if (MeAlcanzaEnergiaYpasos(nivelDeEnergiaActual, mapa, iOrigen,
						jOrigen, numPasos, maxPasos)) {
					AgregoPaso(caminoActual, iOrigen, jOrigen);
					nivelDeEnergiaActual = DescargoEnergia(mapa, iOrigen,
							jOrigen, nivelDeEnergiaActual);
					numPasos++;
					if (Exito(iOrigen, jOrigen, iDestino, jDestino)) {
						...
						for (int x = iOrigen - 1; x < iOrigen + 2; x++) {
							for (int y = jOrigen - 1; y < jOrigen + 2; y++) {
								explorador(x, y, iDestino, jDestino, numPasos,
										maxPasos, nivelDeEnergiaActual, datos,
										caminoActual, mejoresCaminos, mapa, n,
										m);
							}
						}
						numPasos--;
	...
	private boolean MeAlcanzaEnergiaYpasos(int nivelDeEnergiaActual,
			int[][] mapa, int x, int y, int numPasos, int maxPasos) {
	...
	private int DescargoEnergia(int[][] mapa, int x, int y,
			int nivelDeEnergiaActual) {
	 ...
	 */
	@Override
	public int nutricionista(int[] menu, int cantidadPlatos,
			int caloriasMaxima, int[][] tablaMenu, boolean[] menuOptimo) {
		// TODO Auto-generated method stub

		tablaMenu = completarTablaMenu(menu, cantidadPlatos, caloriasMaxima,
				tablaMenu);
	...
	private int[][] completarTablaMenu(int[] menu, int cantidadPlatos,
			int caloriasMaxima, int[][] tablaMenu) {
		...
		for (int i = 1; i < cantidadPlatos; i++) {
			for (int j = 1; j < caloriasMaxima + 1; j++) {
				...
				if (j < menu[i]) {
					...
					if (tablaMenu[i - 1][j] > tablaMenu[i - 1][j - menu[i]]
							+ menu[i]) {
						...
						tablaMenu[i][j] = tablaMenu[i - 1][j - menu[i]]
								+ menu[i];
					}
				}
	...
	public static int[] cantidadObjetos(int[] objetos, int cantidadObjetos,
			int[][] solucion, int pesoMaximo) {
	...
	private int[][] generarTabla(int cantidadPlatos, int caloriasMaxima,
			int[][] tablaMenu) {
